...
function getGitHubInput() {
  return {
    ...
    repoUrl:
      core.getInput('repo-url') || (process.env.GITHUB_REPOSITORY as string),
...
async function main() {
  ...
  if (MANIFEST_COMMANDS.includes(command as string)) {
    return await runManifest(command as string)
  }
  const github = await getGitHubInstance()
...
function getGitHubInstance() {
  ...
  let proxy
  if (proxyServer) {
    const [host, port] = proxyServer.split(':') as [string, string]
...
async function manifestInstance(github: GitHub) {
  ...
  const bumpPatchForMinorPreMajor = getOptionalBooleanInput(
  ...
  const pullRequestTitlePattern =
    core.getInput('pull-request-title-pattern') || undefined
  ...
  return await Manifest.fromConfig(
    ...
    },
    {
      ...
      releaseSearchDepth:
        typeof releaseSearchDepth === 'string'
          ? parseInt(releaseSearchDepth)
          : releaseSearchDepth,
      commitSearchDepth:
        typeof commitSearchDepth === 'string'
          ? parseInt(commitSearchDepth)
          : commitSearchDepth
    },
    path
  )
...
function outputReleases(releases: any) {
  releases = releases.filter((release: any) => release !== undefined)
  const pathsReleased = []
  if (releases.length) {
...
function outputPRs(prs: any) {
  prs = prs.filter((pr: any) => pr !== undefined)
  if (prs.length) {
...
async function run(): Promise<void> {
  try {
    const type: string = core.getInput('release-type')
    core.debug(`releas-type ${type}`)
    await main()
  } catch (error) {
